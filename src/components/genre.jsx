  ...
  return (
    <ul className="list-none pl-0 text-white text-[12px] font-normal flex items-center gap-[10px] flex-wrap mt-[32px] justify-center">
      {displayedGenres.map((genre, index) => (
        <li key={index} onClick={() => handleGenreClick(genre)} className="bg-[#222C4F] px-[12px] py-[6px] rounded-[8px] cursor-pointer text-[12px] opacity-[0.8] transition-transform duration-300 ease-in-out hover:bg-[#1a223d] hover:scale-105">{genre}</li>
      ))}
      {!showAll && (
        <li onClick={() => setShowAll(true)} className="bg-[#222C4F] px-[12px] py-[6px] rounded-[8px] cursor-pointer text-[12px] opacity-[0.8] transition-transform duration-300 ease-in-out hover:bg-[#1a223d] hover:scale-105">
